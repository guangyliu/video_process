...
class MyDetector(ContentDetector):
    ...
    def __init__(
        ...
        """
        super().__init__()
        self._threshold: list = threshold
        self.de_upper = threshold[0]
        self.dl_upper = threshold[1]
        print(threshold)
    ...
    def process_frame(self, frame_num: int, frame_img: numpy.ndarray) -> List[int]:
        ...
        # self.stats_manager.
        _ = self._calculate_frame_score(frame_num, frame_img)
        delta_edge = self.stats_manager.get_metrics(frame_num,['delta_edges'])[0]
        delta_lum = self.stats_manager.get_metrics(frame_num, ['delta_lum'])[0]
        if delta_edge is None or delta_lum is None:
            return []
        self._frame_score = (delta_edge > self.de_upper) or (delta_lum > self.dl_upper)
        if not self._frame_score:
            return []
        ...
        if self._frame_score  and min_length_met:
            self._last_scene_cut = frame_num
            return [frame_num]
...
class MyDetectorV2(MyDetector):
    ...
    def __init__(
        ...
        self.de_upper = threshold[0]
        self.dl_upper = threshold[1]
        self.prev_scene_maneger = prev_scene_maneger
        print(threshold)
        self._min_scene_len: int = min_scene_len
        self._last_scene_cut: Optional[int] = None
        self._last_frame: Optional[ContentDetector._FrameData] = None
